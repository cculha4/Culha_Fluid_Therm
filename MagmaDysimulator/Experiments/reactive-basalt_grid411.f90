...
!-----------------------------------------------------------------------------
     ...
     program FVNS
        ...
        allocatable :: x(:),y(:),uin(:),u(:,:),v(:,:),p(:,:), &
                       ...
                       cnctr(:,:),xlct(:,:),ylct(:,:),u_lct(:,:),v_lct(:,:),phi_T(:,:),phivol10av(:,:)
        ...
        nsl = 731!16*nsp/5  !percent of crystal at the top boundary
        ...
        allocate(x(nx),y(ny),uin(ny+1),u(nx,ny),v(nx,ny),Tem(nx,ny), &
                ...
                rhog(nx,ny),xmug(nx,ny),phi_eq(nx,ny),xo_new(10000),yo_new(10000),&
                 ...
                 ns_needed_before(int(dble(nx)/dble(nav)),int(dble(ny)/dble(nav))),&
                 ns_needed(int(dble(nx)/dble(nav)),int(dble(ny)/dble(nav))),&
                 ns_in_box(int(dble(nx)/dble(nav)),int(dble(ny)/dble(nav))),&
                 phi_T(int(dble(nx)/dble(nav)),int(dble(ny)/dble(nav))),&
                 phivol10av(int(dble(nx)/dble(nav)),int(dble(ny)/dble(nav))))
...
!------------Adding or subtracting crystals--------------------------
            ...
            call determining_volume(Tem,phigas,phi_eq,ns_needed,iout,lm,iout_max,t,&
                                     tmax,xout,nwrite,nav,rdx,xo,yo,num,&
                                     p_vol,ns_in_box,phi_T,phivol10av)
...
!          call concentration_tracking(navxlct,navylct,xlct,ylct,u,v,u_lct,v_lct,dt)
           ...
           if (lm.eq.iout_max.or.t.ge.tmax.or.t.ge.xout(iout)) then
              ...
              open(10,file="data_magma_chamber_phi_trc_" // nss // "_" // nsc //"_" // num // ".dat")
               do k = 1,int(dble(ny)/dble(nav))
                   do l =1,int(dble(nx)/dble(nav))
                      write(10,*) phi_eq_before(l,k),ns_needed_before(l,k),&
                                  phi_eq(l,k),ns_needed(l,k),phi_T(l,k),phivol10av(l,k)
                   end do
               end do
              close(10)
...
!find the random value
      ...
      subroutine determining_volume(Tem,sliq,phi_eq,ns_needed,iout,lm,iout_max,&
                                    t,tmax,xout,nwrite,nav,rdx,xo,yo,mboa,p,ns_in_box,phi_T,phivol10av)
...
!          !!!!!! Out put is the volume of crystals present in the cell (phi_eq) and the number of crystals needed to get to that equilibrium (ns_needed)
          ...
          dimension :: Tem(nx,ny),sliq(nx,ny),phi_eq(nx,ny),xout(nwrite),&
                       xo(10000),yo(10000),p(4),phi_T(nx,ny),phivol10av(nx,ny)
          integer, dimension(int(dble(nx)/dble(nav)),int(dble(ny)/dble(nav))) :: ns_needed,ns_in_box
...
!          ! get as many random number as you want by change 15000 below
          allocate(phivol(nx,ny),phivol10(nx,ny),Tem10(nx,ny),&
                  Tem10av(nx,ny),ks(nx*ny),nxs(nx*ny),nys(nx*ny),phivf(nx,ny,3),&
                  phivf1(nx*ny),sliq10av(nx,ny),sliq10(nx,ny),x(nx,3,3),y(ny,3,3),s_part(nx,ny),&
