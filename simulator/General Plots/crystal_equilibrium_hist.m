...
% h1 = histogram(BigDT./(max(Tempin)-min(Tempin)),30,'FaceColor',[230,97,1]./255,'FaceAlpha',.7,'Normalization','probability')
% h1.Normalization = 'probability';
%  h1.BinWidth = .05;
hold on
% boxplot(BigDT./(max(Tempin)-min(Tempin)))
...
% boxplot(BigDT_end./(max(Tempin)-min(Tempin)))
% h1.XLim = [-.6, 0.2];
% h2.XLim = [-.6, 0.2];
% h2 = histogram(BigDT_end./(max(Tempin)-min(Tempin)),30,'FaceColor',[94,60,153]./255,'FaceAlpha',.7,'Normalization','probability')
