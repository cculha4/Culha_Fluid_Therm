...
Thot = 1160;
Pressure = 2000; %MPa
gas = 0; %1 if there is gas present
...
minTsol = min(T_liq_sol);%800;%816.5;%min(T_liq_sol);
...
if exist(nm,'file')==2
    ...
    input = importdata(filename,deliniator);
    if isempty(input)==0
        T_am = input.data(:,2); minTsol =  min(T_am);
        iminTsol = find(round(T_am,2)==round(minTsol,2));
        index = zeros(size(T_am(1:iminTsol(1))));
        for i = 1:numel(index)
        ...
        rho_am(index) = input.data(1:iminTsol(1),6);
        mass_am(index) = input.data(1:iminTsol(1),5);
        vol_am(index) = input.data(1:iminTsol(1),29);
...
%     input = importdata(filename,deliniator);
%     if isempty(input)==0
%         T_ap = input.data(:,2);  minTsol =  min(T_ap);
%         iminTsol = find(round(T_ap,2)==round(minTsol,2));
%         index = zeros(size(T_ap(1:iminTsol(1))));
%         for i = 1:numel(index)
...
%         rho_ap(index) = input.data(1:iminTsol(1),6);
%         mass_ap(index) = input.data(1:iminTsol(1),5);
%         vol_ap(index) = input.data(1:iminTsol(1),29);
%         %calculate the volume and mass and hence the density of the solids
...
if exist(nm,'file')==2
    ...
    input = importdata(filename,deliniator);
    if isempty(input)==0
        T_cm = input.data(:,2);  minTsol =  min(T_cm);
        iminTsol = find(round(T_cm,2)==round(minTsol,2));
        index = zeros(size(T_cm(1:iminTsol(1))));
        for i = 1:numel(index)
        ...
        rho_cm(index) = input.data(1:iminTsol(1),6);
        mass_cm(index) = input.data(1:iminTsol(1),5);
        vol_cm(index) = input.data(1:iminTsol(1),29);
        %calculate the volume and mass and hence the density of the solids
...
if exist(nm,'file')==2
    ...
    if isempty(input)==0
        ...
        T_fa = input.data(:,2);minTsol =  min(T_fa);
        iminTsol = find(round(T_fa,2)==round(minTsol,2));
        index = zeros(size(T_fa(1:iminTsol(1))));
        for i = 1:numel(index)
        ...
        rho_fa(index) = input.data(1:iminTsol(1),6);
        mass_fa(index) = input.data(1:iminTsol(1),5);
        vol_fa(index) = input.data(1:iminTsol(1),29);
        %calculate the volume and mass and hence the density of the solids
...
if exist(nm,'file')==2
    ...
    input = importdata(filename,deliniator);
    if isempty(input)==0
        T_ga = input.data(:,2);  minTsol =  min(T_ga);
        iminTsol = find(round(T_ga,2)==round(minTsol,2));
        index = zeros(size(T_ga(1:iminTsol(1))));
        for i = 1:numel(index)
        ...
        rho_ga(index) = input.data(1:iminTsol(1),6);
        mass_ga(index) = input.data(1:iminTsol(1),5);
        vol_ga(index) = input.data(1:iminTsol(1),29);
        %calculate the volume and mass and hence the density of the solids
...
if exist(nm,'file')==2
    ...
    input = importdata(filename,deliniator);
    if isempty(input)==0
        T_gr = input.data(:,2);  minTsol =  min(T_gr);
        iminTsol = find(round(T_gr,2)==round(minTsol,2));
        index = zeros(size(T_gr(1:iminTsol(1))));
        for i = 1:numel(index)
        ...
        rho_gr(index) = input.data(1:iminTsol(1),6);
        mass_gr(index) = input.data(1:iminTsol(1),5);
        vol_gr(index) = input.data(1:iminTsol(1),29);
        %calculate the volume and mass and hence the density of the solids
...
if exist(nm,'file')==2
    ...
    input = importdata(filename,deliniator);
    if isempty(input)==0
        T_cli = input.data(:,2); minTsol =  min(T_cli);
        iminTsol = find(round(T_cli,1)==round(minTsol,1));
        index = zeros(size(T_cli(1:iminTsol(1))));
        for i = 1:numel(index)
        ...
        rho_cli(index) = input.data(1:iminTsol(1),6);
        mass_cli(index) = input.data(1:iminTsol(1),5);
        vol_cli(index) = input.data(1:iminTsol(1),29);
        %calculate the volume and mass and hence the density of the solids
...
if exist(nm,'file')==2
    ...
    input = importdata(filename,deliniator);
    if isempty(input)==0
        T_f = input.data(:,2);minTsol =  min(T_f);
        iminTsol = find(round(T_f,2)==round(minTsol,2));
        index = zeros(size(T_f(1:iminTsol(1))));
        for i = 1:numel(index)
        ...
        rho_f(index) = input.data(1:iminTsol(1),6);
        mass_f(index) = input.data(1:iminTsol(1),5);
        vol_f(index) = input.data(1:iminTsol(1),29);
        %calculate the volume and mass and hence the density of the solids
...
if exist(nm,'file')==2
    ...
    input = importdata(filename,deliniator);
    if isempty(input)==0
        T_ol = input.data(:,2); minTsol = min(T_ol);
        iminTsol = find(round(T_ol,2)==round(minTsol,2));
        index = zeros(size(T_ol(1:iminTsol(1))));
        for i = 1:numel(index)
        ...
        rho_ol(index) = input.data(1:iminTsol(1),6);
        mass_ol(index) = input.data(1:iminTsol(1),5);
        vol_ol(index) = input.data(1:iminTsol(1),29);
        %calculate the volume and mass and hence the density of the solids
...
if exist(nm,'file')==2
    ...
    input = importdata(filename,deliniator);
    if isempty(input)==0
        T_wh = input.data(:,2);  minTsol =  min(T_wh);
        iminTsol = find(round(T_wh,2)==round(minTsol,2));
        index = zeros(size(T_wh(1:iminTsol(1))));
        for i = 1:numel(index)
        ...
        rho_wh(index) = input.data(1:iminTsol(1),6);
        mass_wh(index) = input.data(1:iminTsol(1),5);
        vol_wh(index) = input.data(1:iminTsol(1),29);
        %calculate the volume and mass and hence the density of the solids
...
if exist(nm,'file')==2
    ...
    input = importdata(filename,deliniator);
    if isempty(input)==0
        T_or = input.data(:,2); minTsol =  min(T_or);
        iminTsol = find(round(T_or,2)==round(minTsol,2));
        index = zeros(size(T_or(1:iminTsol(1))));
        for i = 1:numel(index)
        ...
        rho_or(index) = input.data(1:iminTsol(1),6);
        mass_or(index) = input.data(1:iminTsol(1),5);
        vol_or(index) = input.data(1:iminTsol(1),29);
        %calculate the volume and mass and hence the density of the solids
...
if exist(nm,'file')==2
    ...
    input = importdata(filename,deliniator);
    if isempty(input)==0
        T_q = input.data(:,2); minTsol =  min(T_q);
        iminTsol = find(round(T_q,2)==round(minTsol,2));
        
        index = zeros(size(T_q(1:iminTsol(1))));
        for i = 1:numel(index)
        ...
        rho_q(index) = input.data(1:iminTsol(1),6);
        mass_q(index) = input.data(1:iminTsol(1),5);
        vol_q(index) = input.data(1:iminTsol(1),29);
        %calculate the volume and mass and hence the density of the solids
...
if exist(nm,'file')==2
    ...
    input = importdata(filename,deliniator);
    if isempty(input)==0
        T_sp = input.data(:,2); minTsol =  min(T_sp);
        iminTsol = find(round(T_sp,2)==round(minTsol,2));
        
        index = zeros(size(T_sp(1:iminTsol(1))));
        for i = 1:numel(index)
        ...
        rho_sp(index) = input.data(1:iminTsol(1),6);
        mass_sp(index) = input.data(1:iminTsol(1),5);
        vol_sp(index) = input.data(1:iminTsol(1),29);
        %calculate the volume and mass and hence the density of the solids
...
begingas = find(rho_gas>0.03); c_g_h = max(T_liq(begingas));
index_g_h = index_h;
if gas
...
p_rho_gas = gas.*polyfit(T_liq(index_g_h:index_c),rho_gas(index_g_h:index_c),10);
...
p_mass_gas = gas.*polyfit(T_liq(index_g_h:index_c),mass_gas(index_g_h:index_c),10);
...
p_H2O_gas = gas.*polyfit(T_liq(index_g_h:index_c),H2O_gas(index_g_h:index_c),10);

p_CO2_liq = polyfit(T_liq(index_h:index_c),CO2_liq(index_h:index_c),10);

p_CO2_gas = gas.*polyfit(T_liq(index_g_h:index_c),CO2_gas(index_g_h:index_c),10);
...
v_rho_gas=polyval(p_rho_gas,T_liq(index_g_h:index_c));
...
v_mass_gas = polyval(p_mass_gas,T_liq(index_g_h:index_c));

v_mass_sol = polyval(p_mass_sol,T_liq(index_h:index_c));
v_H2O_liq = polyval(p_H2O_liq,T_liq(index_h:index_c));

v_H2O_gas = polyval(p_H2O_gas,T_liq(index_g_h:index_c));

v_CO2_liq = polyval(p_CO2_liq,T_liq(index_h:index_c));

v_CO2_gas = polyval(p_CO2_gas,T_liq(index_g_h:index_c));
...
if ~isempty(trash)
    for n = 1:numel(trash)
     vol_liq_n1(trash(n)) = vol_liq_n1(trash(n)-1);%mean([vol_liq_n1(trash(n)+1) vol_liq_n1(trash(n)-1)]);
    end
end
...
if ~isempty(trash)
    for n = 1:numel(trash)
       vol_gas_n1(trash(n)) = vol_gas_n1(trash(n)-1);%mean([vol_gas_n1(trash+1) vol_gas_n1(trash-1)]);
    end
end
...
if ~isempty(trash)
    for n = 1:numel(trash)
       vol_sol_tbl1(trash) = vol_sol_tbl1(trash-1);%mean([vol_sol_tbl1(trash+1) vol_sol_tbl1(trash-1)]);
    end
end
